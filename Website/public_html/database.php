<?php
    ...
    /**
     *Search all companies from database with the matching name (For search bars)
     *
     * @param string The with the name or part of the name of a company
     * @return array list of companies
     */
    function searchCompanies($param)
    {
        $companies = array();
        $pdo = openConnection();
        $query = "SELECT * FROM employer WHERE company_name LIKE '%$param%'";
        try {
            $res = $pdo->query($query);
            $pdo = null;

            while($row = $res->fetch(PDO::FETCH_ASSOC)) {
                $employerId = htmlspecialchars($row["employerId"]);
                $companyName = htmlspecialchars($row["companyName"]);
                $companyHq = htmlspecialchars($row["companyHq"]);
                $companyUrl = htmlspecialchars($row["companyUrl"]);
                $company = new Employer($employerId, $companyName, $companyHq, $companyUrl);
